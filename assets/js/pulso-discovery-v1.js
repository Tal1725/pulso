...
(function(){
 ...
 let mounting=false;
 function css(){if(document.getElementById('pulso-discovery-v4-css'))return;const s=document.createElement('style');s.id='pulso-discovery-v4-css';s.textContent='.pulso-discovery{margin:0 0 12px;padding:14px 16px;border:1px solid rgba(255,255,255,.08);border-radius:16px;background:linear-gradient(135deg,rgba(255,255,255,.045),rgba(255,75,134,.055))}.pulso-discovery-chips{display:flex;gap:7px;overflow:auto;padding-top:11px;scrollbar-width:none}.pulso-discovery-chip{white-space:nowrap;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.035);color:#ddd;padding:7px 10px;border-radius:999px;font-size:12px}.pulso-discovery-chip.active{border-color:rgba(255,75,134,.65);color:#fff;background:rgba(255,75,134,.12)}.pulso-discovery-clear{border:0;background:transparent;color:#ff4b86;font-weight:800;font-size:12px;cursor:pointer}.pulso-discovery-result{margin:0 0 10px;padding:9px 12px;border:1px solid rgba(255,255,255,.08);border-radius:12px;color:#aeb4c2;font-size:12px;background:rgba(255,255,255,.025)}';document.head.appendChild(s)}
 async function getUser(){const sb=getSB();if(!sb)return null;const {data}=await sb.auth.getSession();return data.session?.user||null}
 async function interests(uid){const sb=getSB();const {data:rows}=await sb.from('pulso_user_interests').select('interest_id').eq('user_id',uid);const ids=(rows||[]).map(x=>x.interest_id);if(!ids.length)return [];const {data}=await sb.from('pulso_interests').select('id,name').in('id',ids);return (data||[]).map(x=>x.name).filter(Boolean)}
 async function mount(){
  if(mounting||document.querySelector('.pulso-discovery'))return;
  const feed=document.getElementById('feed');if(!feed)return;
  mounting=true;
  try{
   const uid=(await getUser())?.id;if(!uid)return;
   if(document.querySelector('.pulso-discovery'))return;
   css();const names=await interests(uid);const list=names.length?names:Object.keys(terms).slice(0,8);
   if(document.querySelector('.pulso-discovery'))return;
   const box=document.createElement('section');box.className='pulso-discovery';
   box.innerHTML='<div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><div style="font-weight:900;font-size:18px">Descubra seu PULSO</div><div style="color:#9da1b1;font-size:12px;margin-top:3px">Encontre conteúdo real sobre o que você gosta.</div></div><button class="pulso-discovery-clear" type="button">Ver tudo</button></div><div class="pulso-discovery-chips">'+list.map(n=>'<button class="pulso-discovery-chip '+(names.includes(n)?'active':'')+'" type="button" data-interest="'+n.replace(/"/g,'&quot;')+'">'+n+'</button>').join('')+'</div>';
   feed.parentNode.insertBefore(box,feed);
   box.querySelector('.pulso-discovery-clear').onclick=()=>{box.querySelectorAll('.active').forEach(x=>x.classList.remove('active'));clear()};
   box.querySelectorAll('[data-interest]').forEach(b=>b.onclick=()=>{box.querySelectorAll('.active').forEach(x=>x.classList.remove('active'));b.classList.add('active');apply(b.dataset.interest)});
 ...
 document.addEventListener('pulso-feed-rendered',()=>{dedupe();setTimeout(()=>{dedupe();mount()},50)});
