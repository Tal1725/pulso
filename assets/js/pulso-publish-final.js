/* PULSO — PUBLICADOR V8 — publicação única */
(() => {
  'use strict';
  ...
  const BUCKET='pulso-videos', MAX_FILE=100*1024*1024;
  ...
  async function publish(){
    ...
    if(f&&f.size>MAX_FILE)return msg('A mídia deve ter no máximo 100 MB.',true);
    ...
    }catch(e){console.error('[PULSO V8]',e);msg('❌ '+(e?.message||'Não foi possível publicar.'),true)}
    finally{publishing=false;b.disabled=false;b.textContent='Publicar'}
  }
  window.pulsoPublish=publish;
  function bind(){const b=ensureComposer();if(!b||b.dataset.pulsoV8)return;b.dataset.pulsoV8='1';b.addEventListener('click',e=>{e.preventDefault();publish()})}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind,{once:true});else bind();
  new MutationObserver(bind).observe(document.documentElement,{childList:true,subtree:true});
  window.pulsoPublisherVersion='v8';
})();