...
async function subscribeInbox(){if(!me)return;try{await sb.realtime.setAuth()}catch{}if(inbox)await sb.removeChannel(inbox);inboxReady=new Promise(resolve=>{inbox=sb.channel(`pulso-call-inbox:${me}`,{config:{private:true}}).on('broadcast',{event:'call'},({payload})=>handleInbox(payload)).subscribe((status,err)=>{if(status==='SUBSCRIBED')resolve(true);if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'){console.warn('PULSO call inbox',err);resolve(false)}})});
}
...
function addVideoButton(){
 ...
 const ready=!!contact;
 ['#videoCallLaunch','#videoCallLaunchHeader'].forEach(sel=>{const b=$(sel);if(b){b.onclick=startCall;b.disabled=!ready;b.style.opacity=ready?'1':'.55';b.title=ready?'Iniciar chamada de vídeo':'Abra uma conversa e selecione um contato primeiro'}})
}
...
function updateVideoButtons(){const ready=!!contact;['#videoCallLaunch','#videoCallLaunchHeader'].forEach(sel=>{const b=$(sel);if(b){b.disabled=!ready;b.style.opacity=ready?'1':'.55';b.title=ready?'Iniciar chamada de vídeo':'Abra uma conversa e selecione um contato primeiro'}})}
(async()=>{ensureUI();bind();try{await auth();if(me)await subscribeInbox()}catch(e){console.warn('PULSO video-call init',e)}sb.auth.onAuthStateChange(async(_event,session)=>{me=session?.user?.id||null;if(me){try{await subscribeInbox()}catch(e){console.warn('PULSO video-call auth',e)}}});})();
